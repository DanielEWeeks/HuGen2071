...
# Entering the name of the function prints it out:
...
```{webr-r}
convert_kg_to_pounds(get_mass_from_length(12, 10.95, 2.64))
```
...
* Scope: variables are local to the function (pass by value).

```{webr-r}
square <- function (number = 7) {
...
Define these two variables in the global environment:
...
Were they changed when we ran the `square()` function?
...
## Scope: pass all needed variables as arguments

* Scope: If a variable is not found in the argument list, it is taken
...
* Best to avoid using global variables within functions: pass all
needed variables as arguments.
   - Do not use any global variables within functions.

## Key points: Writing R functions
...
* Annotate your functions with clear comments.

* **Do not use global variables** within your function
  - Pass all needed variables as arguments
* May be useful to specify default values for arguments.
* Argument matching is done by name (full or partial) or by
position
   - If omitted, the default value is used.
* Typing the name of a non-object-oriented function will print out its code.
